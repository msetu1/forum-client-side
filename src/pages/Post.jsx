...
const Post = ({ post,refetch }) => {
  let { authorImg, upVote, downVote, authorName, postImg, _id, tag, postTime } =
    post || {};
  ...
  const handleUpVoteButton = async (id) => {
    ...
    const voteCounts = { upVote, downVote };
    const res = await axiosPublic.patch(`/posts/${id}`, voteCounts);
    if (res?.data) {
      refetch()
    }
      ...
      <div className="card px-5  bg-base-100 shadow-xl">
        ...
        <div className="">
          ...
          <div className="flex justify-between text-xl my-5">
            <button className="btn" onClick={() => handleUpVoteButton(_id)}>
              up vote({upVote})
            </button>
            <button className="btn" onClick={() => handleDownVoteButton(_id)}>
              down vote({downVote})
            </button>
